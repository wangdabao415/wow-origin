 ...
 * - tag: 标签分类 (16=内地, 17=港台, 18=欧美, 19=韩国, 20=日本, 可选，默认16)
 *
 * 返回格式:
 * {
 *   mvs: [...],
...
module.exports = (query, request) => {
  ...
  const tag = parseInt(query.tag) || 16       // 标签分类(默认内地)
  ...
  return request('MvService.MvInfoProServer', 'GetHotMv', data, {
    uin: 0,
    qm_keyst: ''
  }).then(res => {
    // 检查响应数据
    ...
    // 错误处理
    throw new Error(`QQ Music hot MV API error: ${error.message || 'Unknown error'}`)
  })
}
